...
_OSEA.QRSDet.argtypes = (ctypes.c_int, ctypes.c_int)
...
def QRSDet(sample_val, init):
	global _OSEA
	result =  _OSEA.QRSDet(ctypes.c_int(sample_val), ctypes.c_int(init))
	return result
...
def main():
	...
	with open('ecg_full.txt', 'r') as f:
		testip = list(csv.reader(f, delimiter='\t'))
		newarr = [i[1] for i in testip]
		beatTypeList, detectionTimeList = AnalyzeBeatTypeSixSecond(newarr, 200, 1024, 360, 200)
